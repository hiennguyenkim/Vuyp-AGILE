(function () {
    ...
    return source
      .map((registration) => {
        ...
        return {
          ...
          eventId,
          eventCode:
            normalizeString(registration?.eventCode) || matchedEvent?.code || "",
          eventName,
          start:
  ...
  function readState() {
    ...
    const firstPassEvents = normalizeEvents(
      rawEvents,
      rawRegistrations,
    );
    const registrations = normalizeRegistrations(rawRegistrations, firstPassEvents);
    const events = normalizeEvents(firstPassEvents, registrations);
    const legacyHistory = buildLegacyHistory(readJson(STORAGE_KEYS.history, []));
  ...
  function registerStudent(eventId, student) {
    ...
    const normalizedStudentId = normalizeString(student?.studentId).toLowerCase();
    ...
    const registrations = sortRegistrations([registration, ...state.registrations]);
    const events = [...state.events];
    events[eventIndex] = {
      ...event,
  ...
  function getEventById(eventId) {
    const state = readState();
    return (
      state.events.find((event) => normalizeString(event.id) === normalizeString(eventId)) ||
      null
    );
  }
